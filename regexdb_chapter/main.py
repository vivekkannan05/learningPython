...
def normalize_text(text: str) -> str:
    """Clean up common PDF text extraction artifacts like broken words."""
    text = re.sub(r'Answ\s+er:', 'Answer:', text)
    return text
...
def read_pdf_and_store(pdf_filename: str, page_number: int, regex_config: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            ensure_table_exists(conn, "question_bank")
            ...
            for match in questions:
                ...
                insert_data_into_database(conn, "question_bank", data)
...
def read_all_pages_and_store(pdf_filename: str, regex_config: dict) -> None:
    content_folder = get_content_folder()
    pdf_path = content_folder / pdf_filename
    ...
    try:
        ...
        try:
            ...
            for chapter_name in chapter_order:
                combined_text = chapter_texts[chapter_name]
                questions = list(re.finditer(regex_config["question_regex"], combined_text))

                if not questions:
                    print(f"  {chapter_name}: No questions found.")
                    continue

                print(f"  {chapter_name}: {len(questions)} question(s)")
            ...
            print(f"\nSuccessfully inserted {total_questions} question(s) across {total_pages} page(s).")
        except (MySQLError, ConnectionError, ValueError) as e:
...
def parse_args():
    parser = argparse.ArgumentParser(description="PDF Question Extractor and Question Bank")
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    extract_parser = subparsers.add_parser("extract", help="Extract questions from a specific PDF page")
    extract_parser.add_argument("page_number", type=int, help="The page number to extract text from.")

    subparsers.add_parser("extract-all", help="Extract questions from all PDF pages")

    chapter_parser = subparsers.add_parser("chapter", help="Load questions by chapter")
    chapter_parser.add_argument("subject_name", type=str, help="The subject name to filter by.")
    chapter_parser.add_argument("chapter_name", type=str, help="The chapter name to load questions from.")

    return parser.parse_args()

def load_questions_by_chapter(subject_name: str, chapter_name: str) -> None:
    ...
    try:
        ...
        query = "SELECT question_text, answer_options, answer FROM question_bank WHERE subject_name = %s AND chapter_name = %s"
        cursor.execute(query, (subject_name, chapter_name))
        questions = cursor.fetchall()
        cursor.close()

        if not questions:
            print(f"No questions found for subject: '{subject_name}', chapter: '{chapter_name}'")
            return
        print(f"Found {len(questions)} question(s) for subject: '{subject_name}', chapter: '{chapter_name}'")
        for i, question in enumerate(questions, 1):
            print(f"\nQ{i}. {question['question_text']}")
            for opt in question['answer_options'].split("\t"):
                print(f"  {opt}")
...
def main():
    ...
    if args.command == "chapter":
        load_questions_by_chapter(args.subject_name, args.chapter_name)
        return

    if args.command is None:
        print("No command provided. Use 'extract' or 'extract-all' or 'load'.")
    ...
    try:
        if args.command == "extract":
            read_pdf_and_store(pdf_filename, args.page_number, regex_config)
        elif args.command == "extract-all":
            read_all_pages_and_store(pdf_filename, regex_config)
        print("\nDone!")
